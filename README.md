...
**▶️ 演示**：克隆仓库后按下方「本地启动」运行；前端 http://localhost:9280 ，后端 OpenAPI http://localhost:8028/docs 。

**⚖️ 合规**：请仅下载你拥有保存权利或已获授权的内容；遵守各平台服务条款与当地法规。本项目不提供 DRM 绕过或未授权下载能力。
...
- **⚙️ 设置**（保存在浏览器 localStorage）：后端 API 地址（可选）、OpenAI API Key / Base URL / 模型、哔哩哔哩 Cookie（可选）。也可在服务端 `backend/.env` 配置（见下）。
